...
public class TeamsAdapter {
    ...
    public void setStatus(String hTeam, String vTeam, int hScore, int vScore) throws SQLException {
        ...
        if(hScore > vScore) {
            String update = "UPDATE Teams " + " SET Wins = Wins + 1" + " WHERE TeamName = '" +  hTeam + "'";
            String update1 = "UPDATE Teams" + " SET Losses = Losses + 1" + " WHERE TeamName = '" + vTeam + "'";
            stmt.executeUpdate(update);
            stmt.executeUpdate(update1);
        }

        if(hScore == vScore) {
            String update = "UPDATE Teams" + " SET Ties = Ties + 1" + " WHERE TeamName = '" + hTeam + "'";
            String update1 = "UPDATE Teams" + " SET Ties = Ties + 1" + " WHERE TeamName = '" + vTeam + "'";
            stmt.executeUpdate(update);
            stmt.executeUpdate(update1);
        }
        if(hScore < vScore) {
            String update = "UPDATE Teams" + " SET Losses = Losses + 1" + " WHERE TeamName = '" + hTeam + "'";
            String update1 = "UPDATE Teams" + " SET Wins = Wins + 1" + " WHERE TeamName = '" + vTeam + "'";
            stmt.executeUpdate(update);
            stmt.executeUpdate(update1);
        }
    }
